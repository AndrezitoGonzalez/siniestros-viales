  ...
  <v-card class="product-card" rounded="lg" elevation="0" border>
    ...
    >
      ...
      <v-chip
        ...
        variant="flat"
      >
        {{ severityLabel(product.stock) }} - {{ product.stock }} involucrado{{ product.stock !== 1 ? 's' : '' }}
      </v-chip>
    </v-img>

    <v-card-text class="pb-2">
      <v-chip size="x-small" color="deep-purple-lighten-4" text-color="deep-purple-darken-2" class="mb-2" variant="flat">
        {{ product.category }}
      </v-chip>
      <h3 class="text-subtitle-1 font-weight-bold text-grey-darken-3 mb-1 text-truncate">
        {{ product.name }}
      </h3>
      ...
      <div class="text-caption text-grey-darken-1">Costo estimado de danos</div>
    </v-card-text>
